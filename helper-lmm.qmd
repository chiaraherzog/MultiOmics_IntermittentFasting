...
df <- as.data.frame(longForm(data, colData = c("age_at_consent", "interventionId", "subjectId", "visitId", "time",
...
keep <- c('Minimal model', 'Basic model with BMI', 'Intervention (higher compliance)',
          'Menopause general', 'Menopause interaction')
source(here("src/prepareLME.R"))
out_lmm <- prepareLME(keep)
...
for (x in names(out_lmm)){
  ...
  writexl::write_xlsx(as.data.frame(out_lmm[[x]]),
                      path = paste0("out/Supplementary-Table-S",
                                    file,
                                    ".xlsx")
    )
}
...
Tables can be viewed in supplementary tables [ST6](st6.qmd), [ST7](st7.qmd), [ST8](st8.qmd), [ST9](st9.qmd), and  [ST10](st10.qmd) and on the data portal.