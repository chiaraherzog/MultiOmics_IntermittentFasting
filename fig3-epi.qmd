...
```{r}
out <- plot$data |> 
  dplyr::select(sampletype, visitId, value, subjectId)  |> 
...
cairo_pdf("out/pdf/3d.pdf", width = 2.5, height = 2.5)
print(qt)
dev.off()
```
...
```{r sourcedata.qt}
qt$data |>
  dplyr::select(visitId, rowname, value, subjectId)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/3d.txt')
...
```{r print.line, eval = F}
cairo_pdf("out/pdf/3e.pdf", width = 2.5, height = 2.5)
print(l1)
dev.off()
```
...
cairo_pdf("out/pdf/3f.pdf", width = 2.5, height = 2.5)
print(grimv2)
dev.off()
```
...
out <- ic_pc_cerv$data |> 
  dplyr::select(PC1, PC2, visitId, ic, subjectId)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) 
...
```{r sourcedata.ic.compliance.cerv}
ext_ic$data |> 
  dplyr::select(subjectId, visitId, rowname, value, compliance)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |>   write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
```{r sourcedata.pc.buccal}
scree_buccal$data |>  
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7d.txt')


out <- ic_pc_buccal$data |> 
  dplyr::select(PC1, PC2, visitId, ic, subjectId)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) 
...
```{r sourcedata.buccalbc}
buccalbc$data |> 
  dplyr::select(subjectId, visitId, rowname, value, compliance)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |>  
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7f.txt')
```
...
```{r sourcedata.buccalage}
buccalage$data |> 
  dplyr::select(subjectId, visitId, rowname, value, compliance)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |>  
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7g.txt')
```
...
```{r sourcedata.pc.blood}
scree_blood$data |>  
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7h.txt')


out <- ic_pc_blood$data |> 
  dplyr::select(PC1, PC2, visitId, ic, subjectId)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) 
...
```{r sourcedata.adaptage}
adaptage$data |> 
  dplyr::select(subjectId, visitId, rowname, value, compliance)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |>  
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s7j.txt')
```
...
cairo_pdf(here("figures-pdf/s7.pdf"),
          width = 16,
          height = 13)
print(plot)
dev.off()

ggsave(filename = here("figures-png/figure-s7.png"),
       plot = plot,
       width = 16,
       height = 13,
       units = 'in')
