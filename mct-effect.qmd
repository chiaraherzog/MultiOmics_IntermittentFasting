...
```{r sourcedata.mct.a}
a$data |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/6a.txt')
...
data.frame(variable = plot@row_names_param$labels,
              plot@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/6b.txt')
...
```{r sourcedata.globalmethylation}
p1$data |> 
  dplyr::select(rowname, value, visitId, interventionId) |>
