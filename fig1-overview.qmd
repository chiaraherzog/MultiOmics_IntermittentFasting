...
df.var.1 <- df.var |> 
  ...
  dplyr::filter(!grepl("_plaque_max|imt_max|_hba1c_ifcc|_ft4|_ft3|height", featureid)) |> 
...
iccplot <- df.var.1 |> 
  ...
  scale_colour_manual(values = grDevices::colorRampPalette(cols[c(8, 1,2,3,5,4,6,7)])(13),
                      aesthetics = c('colour', 'fill')) +
  coord_cartesian(xlim = c(0, 1))

# # save colours for re-use elsewhere
levels <- levels(df.var.1$assay)
cols2 <- grDevices::colorRampPalette(cols[c(8, 1,2,3,5,4,6,7)])(13)
names(cols2) = gsub("<b>|</b>", "", levels)
cols_for_assays <- cols2
names(cols_for_assays) <- c("Saliva metabolome", "Urine metabolome",
                            "Stool microbiome", "Blood methylation",
                            "Stool microbiome (ASVs)", "Saliva microbiome",
                            "Cervical methylation", "Body composition",
                            "Functional clinical features", "Saliva microbiome (ASVs)",
...
load(here("out/variance_partition_df_var.Rdata")) # note: revised and this are identical, except one has ASVs added.
...
features <- df.var |>
  ...
  dplyr::filter(!assay %in% c("Saliva microbiome: ASVs","Saliva microbiome: families.clr",
                              "Stool microbiome: ASVs", "Stool microbiome: families.clr",
                              "Saliva nuclear magnetic resonance: clr","Urine nuclear magnetic resonance: clr") & !grepl("ASVs", assay)) |>
  tidyr::separate(featureid, "_", into = c(NA, 'rowname'), remove = F, extra = 'merge') |> 
  dplyr::filter(!grepl("sports|sono", assay) & !rowname %in% exclude & rowname != "height") |> 
...
omes <- c(1, 3, 4, 5, 6, 9, 10, 13:16, 22, 40:41)
df <-  MultiAssayExperiment::longFormat(data[,data$interventionId != "S" & !data$visitId %in% c("M12", "M18"),omes],
...
pc <- FactoMineR::PCA(mat,graph = F,ncp = 5)
...
# Relabel assays
tmp6 <- tmp5 |>
  dplyr::mutate(assay = case_when(grepl("Flow cytometry", assay) ~ assay, 
                                  grepl("magnetic", assay) & grepl("Saliva", assay) ~ "Saliva metabolome",
                                  grepl("magnetic", assay) & grepl("Urine", assay) ~ "Urine metabolome",
...
pcs <- (pc1|pc2|pc3|pc4|pc5) + plot_layout(guides = 'collect') &
  theme(legend.position = 'right')
```
