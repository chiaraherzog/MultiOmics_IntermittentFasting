---
title: "Overview data (F1, Fig. S1-3)"
format:
  html:
    toc: true
    code-fold: true
...
![](figures-png/figure-s1.png)

## Supplementary Figure 2

![](figures-png/figure-s2.png)

## Supplementary Figure 3

![](figures-png/figure-s3.png)
...
```{r sourcedata.compliance.long, eval = F}
out <- compliance2_df |>
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-c(subjectId, shape)) |> 
  dplyr::arrange(id)
...
```{r sourcedata.compliance.all, eval = F}
out <- compliance1_df |>
  dplyr::select(-c(subjectId, shape, color))

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/1cd.txt')
```
...
# In-text numbers (Jul 29)
df.var.1 |> dplyr::group_by(assay) |> 
  dplyr::summarise(med = median(icc,na.rm = T)) |> 
...
df.var.1 |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/1e.txt')
```
...
```{r sourcdata.varfeatures}
features_df |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/1f.txt')
...
```{r}
tab |> dplyr::select(Var1, Var2, Freq, prop, n) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                        file = 'srcdata/s1a.txt')
```
...
cairo_pdf(here("out/pdf/s1c.pdf"),
          width = 1.2,
          height = 2.57)
plot(supplstop)
dev.off()
...
```{sourcdata.supplstop}
tmp_df |> dplyr::select(-subjectId) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                        file = 'srcdata/s1c.txt')
```
...
cairo_pdf(here("out/pdf/s1d.pdf"),
          width = 3.5,
          height = 2.57)
plot(suppl)
dev.off()
...
```{r sourcedata.suppl.stop}
tab |>
  dplyr::select(Var1, Var2, Freq, prop, n) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                        file = 'srcdata/s1d.txt')
```
...
cairo_pdf(here("out/pdf/s1e.pdf"),
          width = 2.5,
          height = 2.57)
plot(effect)
dev.off()
...
cairo_pdf(here("out/pdf/s1f.pdf"),
          width = 5,
          height = 5)
plot(wordsplot)
dev.off()
...
```{r print.vas, eval = F}
cairo_pdf(here("out/pdf/s1g.pdf"),
          width = 2.57,
          height = 2.57)
plot(vas)
dev.off()
...
```{r print.eq, eval = F}
cairo_pdf(here("out/pdf/s1h.pdf"),
          width = 3.75,
          height = 2.57)
plot(scores)
dev.off()
...
eq5d5l_df |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-c(subjectId, t)) |>
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
              file = 'srcdata/s1gh.txt')
...
wearable_filtered <- wearable |> 
  dplyr::filter(steps > nonwear_threshold) |> 
  dplyr::mutate(activity_level = ifelse(steps >= active_threshold, "Active", "Sedentary"))

d_int  <- data.table::as.data.table(interval_windows)
d_days <- data.table::as.data.table(wearable_filtered)

data.table::setkey(d_int, subjectId, start, end)
...
interval_summary |> 
  ...
  dplyr::group_by(visitId) |> 
  dplyr::summarise(med = median(mean_steps))
...
```{r print.ex, eval = F}
cairo_pdf(here("out/pdf/s1i.pdf"),
          width = 2.95,
          height = 2.57)
plot(plot)
dev.off()
...
out <- interval_summary |> 
  dplyr::filter(interventionId != 'S') |> 
  dplyr::filter(subjectId %in% complete) |> 
  dplyr::select(subjectId, visitId, mean_steps, interventionId)  |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)

out |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s1i.txt')
```

## Supplementary figure 2

(Reviewer request, UpSet plot)
...
# simple matrix
tmp <- x |> dplyr::filter(assay %in% c("Blood haemogram",
  ...
  dplyr::mutate(assay = case_when(grepl("microbiome", assay) ~ gsub("[:][ ]families", "", assay),
                                  grepl("nuclear", assay) ~ gsub("nuclear[ ]magnetic[ ]resonance", "metabolome", assay),
                                  grepl("methylation", assay) ~ paste0(stringr::str_to_sentence(stringr::str_split_i(assay, "[ ]", 4)), "  methylation"),
                                  grepl("haemo", assay) ~ "Routine bloods",
                                  grepl("Flow", assay) ~ "Immune profiling",
                                  grepl("Skin", assay) ~ "Skin Histology")) |> 
  dplyr::filter(!visitId %in% c("M12", "M18") & !grepl("S", subjectId)) |> 
  dplyr::mutate(set = paste(assay, visitId, sep = '_'),
           present = 1L)

wide <- tmp |> 
  dplyr::select(subjectId, set, present) |> 
  tidyr::pivot_wider(names_from = set, values_from = present, values_fill = 0L,
                     id_cols = subjectId) |> 
  as.data.frame() |> 
  dplyr::select(-subjectId)

library(UpSetR)
up <- upset(wide,nsets = 40, order.by = 'freq',
      mb.ratio = c(0.3, 0.7))
```

```{r}
pdf(file = 'figures-png/figure-s2.png',
    width = 6,
    height = 7)
print(up)
dev.off()
...
```{r}
wide |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s2.txt')
...
```{r sourcedata.pcdat}
out <- pcdat |> dplyr::select(Dim.1, Dim.2, subjectId, interventionId, visitId) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::arrange(id) |> dplyr::select(-subjectId)
...
```{r sourcedata.scree}
out <-scree |> as.data.frame() |> tibble::rownames_to_column('PC') |> 
...
source_data <- map_dfr(1:10, function(pc) {
  Dim <- paste0("Dim.", pc)
  ...
  contrib |>
    ...
    dplyr::transmute(
      ...
      feature = feature,
      assay = assay,
      contribution = .data[[Dim]]
    )
})

source_data |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s3c-l.txt')
```
