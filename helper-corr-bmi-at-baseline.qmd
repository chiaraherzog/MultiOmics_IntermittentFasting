...
library(dplyr)
library(ggplot2)
here::i_am("helper-corr-bmi-at-baseline.qmd")
source("src/corrBmiBaseline.R")
```
...
```{r, eval = F}
corr <- corrBmiBaseline(df)

save(corr, file = "out/corrBmiBaseline.R")
```

# Visualise histogram of p values for correlation with bmi
