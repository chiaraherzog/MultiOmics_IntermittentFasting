...
#### a_1) Bubble plot, I versus K separately
...
#### a_3) Potential diagram - up/down by feature?
...
### d) Arginine and cholesterol

```{r}
ind = 549
m1 = corr$measure1[ind]
m2 = gsub("-log", "", corr$measure2[ind])
m2 = corr$measure2[ind]
lab1 = 'Cholesterol (normalised)'
lab2 = 'Arginine (normalised)'
d <- rmcorr_plot(m1, m2, lab1, lab2)
```

### e) GrimAge and Neutrophils
...
```{r}
f <- rmcorr_diag("AgeAccelGrim")
```

### g) PhenoAge diagram

```{r}
g <- rmcorr_diag("PhenoAge")
```

### h) DunedinPACE diagram

```{r}
h <- rmcorr_diag("Dunedin")
```

### i) DamAge diagram

```{r}
i <- rmcorr_diag("DamAge")
```
...
```{r}
j <- rmcorr_diag("AdaptAge")
```

### k) WID-buccal-BC

```{r}
k <- rmcorr_diag("buccal_BC")
```

### l) WID-BC

```{r}
l <- rmcorr_diag("index_BC")
```

### m) WID-EC

```{r}
m <- rmcorr_diag("index_EC")
```
...
plot <- (a+b+c+d+e+f+g+h+i+j+k+l+m + guide_area()) + plot_layout(design = design,guides = 'collect') +
  plot_annotation(tag_levels = 'a')&
    theme(plot.tag = element_text(face = 'bold'))

cairo_pdf(here("figures-pdf/e10.pdf"), width = 15,
          height = 18)
plot
dev.off()
