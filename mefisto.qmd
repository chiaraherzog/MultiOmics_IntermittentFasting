...
cairo_pdf(file = 'out/pdf/s17a.pdf', width = 5, height = 3.5)
draw(h)
dev.off()
```

```{r sourcedata.17a}
data.frame(variable = rownames(h@matrix),
           h@matrix) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17a.txt')
...
ggsave(plot, file = 'out/pdf/s17b.pdf', width = 6, height = 3.5)

```

```{r sourcedata.s17c}
plot$data |> dplyr::select(-sample) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17b.txt')
```
...
ggsave(p, file = 'out/pdf/s17d.pdf', width = 9, height = 3.5)

p$data |> dplyr::select(subjectId, visitId, value.factor, compliance, group) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |> dplyr::arrange(id)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17c.txt')
 
p <- mefisto_factor_box(df2, metadat, "Factor9")
ggsave(p, file = 'out/pdf/s17e.pdf', width = 9, height = 3.5)

p$data |> dplyr::select(subjectId, visitId, value.factor, compliance, group) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |> dplyr::arrange(id)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17e.txt')

p <- mefisto_factor_box(df2, metadat, "Factor11")
ggsave(p, file = 'out/pdf/s17g.pdf', width = 9, height = 3.5)

p$data |> dplyr::select(subjectId, visitId, value.factor, compliance, group) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId) |> dplyr::arrange(id)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17g.txt')
```
...
f12
cairo_pdf(file = 'out/pdf/s17d.pdf', width = 6, height = 3.5)
print(f12)
dev.off()

f12$data |> dplyr::arrange(dplyr::desc(abs(value))) |> dplyr::slice(1:30) |>
  dplyr::select(feature, value, view)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17d.txt')
  

f9 <- plotTopFactorWeights(model, factorFilter = 9,orientation = 'landscape')
f9
cairo_pdf(file = 'out/pdf/s17f.pdf', width = 6, height = 3.5)
print(f9)
dev.off()

f9$data |> dplyr::arrange(dplyr::desc(abs(value))) |> dplyr::slice(1:30) |>
  dplyr::select(feature, value, view)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17f.txt')

f11 <- plotTopFactorWeights(model, factorFilter = 11,orientation = 'landscape')
f11
cairo_pdf(file = 'out/pdf/s17h.pdf', width = 6, height = 3.5)
print(f11)
dev.off()

f11$data |> dplyr::arrange(dplyr::desc(abs(value))) |> dplyr::slice(1:30) |>
  dplyr::select(feature, value, view)|> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s17h.txt')
```
