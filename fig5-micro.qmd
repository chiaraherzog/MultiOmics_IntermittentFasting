...
# ASV-level M0 feature association with age and menopauze
...
plotAlpha(d2,dmin, paste0("Saliva, high compliance (n=",n, ")"))[[4]] %>%  
  dplyr::rename(id = subjectId) %>%
  mutate(id = as.integer(factor(id))) %>%
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s13a.txt')
plotAlpha(d2,dmin, paste0("Stool, high compliance (n=",n, ")"))[[4]] %>%  
  dplyr::rename(id = subjectId) %>%
  mutate(id = as.integer(factor(id))) %>%
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s13b.txt')
...
datM0 %>%
 tidyr::separate(primary, into = c("id", "visitId"), sep = "(?=M)") %>%
  dplyr::select(-sampleId) %>%
  mutate(id = as.integer(factor(id))) %>%
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s13d.txt')
...
full_join(p1$data,p2$data) %>% 
  dplyr::rename(id = subjectId) %>%
  mutate(id = as.integer(factor(id))) %>%
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s13c.txt')
...
```{r source.5c}
plot_lmm_heatmap_frag_v3(exp = "Saliva microbiome: families", 
                      lmm_data_time = out_lmm_1$`Minimal model`,
                      lmm_data_compliance = out_lmm_1$`Basic model with BMI`,
                      relabel = dat_relab,
                      age_cor = F,
...
df <- longForm(data[,,'Saliva microbiome: families'], colDataCols = c("interventionId", "subjectId", "visitId", "time", "compliance", "comprate", "mpstatrs", "age_at_consent", "bmi_at_consent", "pattern", "smoking_ever", "etohu_curr", "diet", "preg_ever", "intactcurr")) |> 
  as.data.frame() |> 
  dplyr::filter(interventionId != 'S' & ! visitId %in% c("M12", "M18")) |> 
...
```{r sourcedata.5d}
p$data |> dplyr::select(rowname, value, visitId, subjectId, compliance)|>
...
```{r source.5f}
 plot_lmm_heatmap_frag_v3(exp = "Stool microbiome: families", 
                      lmm_data_time = out_lmm_1$`Minimal model`,
                      lmm_data_compliance = out_lmm_1$`Basic model with BMI`,
                      relabel = dat_relab,
                      age_cor = T,
