---
title: "Immune and metabolome data (F4, S8-11)"
format:
  html:
    toc: true
    code-fold: true
...
![](figures-png/figure-s8.png)

## Supplementary Figure 9

![](figures-png/figure-s9.png)

## Supplementary Figure 10

![](figures-png/figure-s10-imm.png)

## Supplementary Figure 11

![](figures-png/figure-s11-metab.png)
...
```{r sourcedata.coef}
ImmAge_gen_coef |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/4a.txt')
```
...
```{r sourcedata.immage.age}
immage$data |> dplyr::select(rowname, value, visitId, interventionId) |> 
...
```{r sourcedata.immage.change}
immage_change$data |> dplyr::select(rowname, value, visitId, subjectId, compliance)|>
...
```{r sourcedata.metab.overview}
m$data |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/4d.txt')
```

### e) Saliva heatmap (Wilcoxon tests)
...
cbind(saliva_heatmap@row_names_param$labels, saliva_heatmap@matrix) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/4e.txt')
```

### f) Urine heatmap (Linear mixed effects models)
...
```{r sourcedata.urine.heatmap}
cbind(urine_heatmap@row_names_param$labels, urine_heatmap@matrix) |> 
  write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/4f.txt')
```
...
```{r print.scree}
cairo_pdf(here("out/pdf/s8a.pdf"),
...
```{r sourcedata.scree.ic}
pc$scree$data |> 
  as.data.frame() |> 
  tibble::rownames_to_column('PC') |> 
  dplyr::select(PC, `percentage of variance`)|> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
cairo_pdf(here("out/pdf/s8b.pdf"),
          width = 3, height = 2.75)
print(pc$biplot)
dev.off()
```

```{r sourcedata.pcbiplot}
out <- pc$biplot$data |> 
  as.data.frame() |> 
  dplyr::mutate(subjectId = substr(name, 1, 4)) |> 
  dplyr::select(subjectId, Col., x, y) |> 
  dplyr::mutate(id = match(subjectId, sample(unique(subjectId)))) |>
  dplyr::select(-subjectId)
...
```{r sourcedata.pcheatmap}
cbind(pc$heatmap@matrix, rownames(pc$heatmap@matrix)) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
cairo_pdf(here("out/pdf/s8d1.pdf"),
...
cairo_pdf(here("out/pdf/s8d2.pdf"),
...
```{r sourcedat.wb_heat}
cbind(wb_heat@row_names_param$labels, wb_heat@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
```{r print.t.pop.tree}
cairo_pdf(here("out/pdf/s8e1.pdf"),
          width = 5.35, height = 5)
print(tree_t)
dev.off()
```
...
```{r sourcedata.t_heat}
cbind(t_heat@row_names_param$labels, t_heat@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
...
```{r}
load("out/immage_out_cv.Rdata")

tmp <- data.frame(age = out$cv_oof$age,
                 cv = out$cv_oof$oof_pred)


labx <- '<b>Age at baseline</b>'
laby <- '<b>ImmuneAge (out-of-sample CV)</b>'
...
```{r}
ggsave(p, file = here("figures-png/figure-s9.png"),
       dpi = 300,
       width = 3.5, height = 3.5)
```

```{r}
p$data |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s9.txt')
...
```{r print.scree.stim}
cairo_pdf(here("out/pdf/s10a.pdf"),
          width = 1.5, height = 2.5)
print(pc$scree)
dev.off()
```

```{r sourcedata.stim.scree}
pc$scree$data |> 
  as.data.frame() |> 
  tibble::rownames_to_column('PC') |> 
  dplyr::select(PC, `percentage of variance`)|> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s10a.txt')
...
cairo_pdf(here("out/pdf/s10b.pdf"),
          width = 4, height = 2.5)
pc$heatmap
dev.off()
```
...
```{r}
cbind(rownames(pc$heatmap@matrix), pc$heatmap@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s10b.txt')
...
cairo_pdf(here("out/pdf/s10c.pdf"),
          width = 6, height = 4.5)
out
dev.off()
```
...
```{r sourcedat.tstim}
cbind(out@row_names_param$labels, out@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s10c.txt')
...
```{r sourcedata.tnf.il17}
tnf$data |> dplyr::select(Flow.cytometry..stimulated.T.cells_lymphocytes_single_cells_live_t_cells_cd8_t_cells_tnf, Immune.age..general_ImmAge_gen, visitId) |> 
  dplyr::rename(TNFa = Flow.cytometry..stimulated.T.cells_lymphocytes_single_cells_live_t_cells_cd8_t_cells_tnf,
                ImmuneAge = Immune.age..general_ImmAge_gen) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s10d.txt')

il17$data |> dplyr::select(Flow.cytometry..stimulated.T.cells_lymphocytes_single_cells_live_t_cells_cd4_t_cells_il17a, Immune.age..general_ImmAge_gen, visitId) |> 
  dplyr::rename(IL17 = Flow.cytometry..stimulated.T.cells_lymphocytes_single_cells_live_t_cells_cd4_t_cells_il17a,
                ImmuneAge = Immune.age..general_ImmAge_gen) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s10e.txt')
```
...
cairo_pdf(here("out/pdf/s11a.pdf"),
          width = 1.5, height = 3)
pc_saliva$scree
dev.off()
```
...
```{r sourcedata.pcsaliva.scree}
pc_saliva$scree$data |> 
  as.data.frame() |> 
  tibble::rownames_to_column('PC') |> 
  dplyr::select(PC, `percentage of variance`)|> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s11a.txt')
```

### b) Saliva: PCA - Heatmap

```{r print.saliva.pca.heatmap}
cairo_pdf(here("out/pdf/s11b.pdf"),
          width = 4, height = 3)
pc_saliva$heatmap
dev.off()
```
...
```{r sourcedata.saliva.heatmap}
cbind(rownames(pc_saliva$heatmap@matrix), pc_saliva$heatmap@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s11b.txt')

```

```{r print.urine.scree}
cairo_pdf(here("out/pdf/s11c.pdf"),
          width = 1.5, height = 3)
pc_urine$scree
dev.off()
```

```{r sourcedata.pcurine.scree}
pc_urine$scree$data |> 
  as.data.frame() |> 
  tibble::rownames_to_column('PC') |> 
  dplyr::select(PC, `percentage of variance`)|> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s11c.txt')
```

### d) Urine: PCA - Heatmap

```{r print.urine.pca.heatmap}
cairo_pdf(here("out/pdf/s11d.pdf"),
          width = 4, height = 3)
pc_urine$heatmap
dev.off()
```
...
```{r sourcedata.urine.heatmap}
cbind(rownames(pc_urine$heatmap@matrix), pc_urine$heatmap@matrix) |> write.table(quote = F, sep = "\t", row.names = F, col.names = T,
                   file = 'srcdata/s11d.txt')
